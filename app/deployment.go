...
func testCRDDeployment(f *framework.Framework) {
	...
	_, err = client.CreateDeployment(context.TODO(), app.NewDpGetOption(clusterID, namespace, DpName), deployment)
	...
	_, err = client.GetDeployment(context.TODO(), app.NewDpGetOption(clusterID, namespace, DpName))
	expect.NoError(err, "Get Deployment Failed")

	err = client.DeleteDeployment(context.TODO(), app.NewDpDeleteOption(clusterID, namespace, DpName))
...
func testListDeployment(f *framework.Framework) {
	...
	for i := 1; i < num; i++ {
		...
		_, err = client.CreateDeployment(context.TODO(), app.NewDpGetOption(clusterID, namespace, dpName[i]), deployment)
		expect.NoError(err, "Create Deployment Failed")
	}

	res, err := client.ListDeployments(context.TODO(), app.NewDpListOption(clusterID, namespace))
	...
	//删除所有创建的 Deployment
	for i := 1; i < num; i++ {
		err = client.DeleteDeployment(context.TODO(), app.NewDpDeleteOption(clusterID, namespace, dpName[i]))
